...
Router.route('/milestones/:_id', {
  ...
  action() {
    ...
    var userTeamId = Teams.findOne({ members: userId });

    if (!userTeamId && !Roles.userIsInRole(userId, [ADMIN, LECTURER, TA])) //go Home you are a ForEverAlone
      this.redirect('home');

    if (!Roles.userIsInRole(userId, [ADMIN, LECTURER, TA]) && !milestone.global && !_.contains(milestone.teams, userTeamId)) {
