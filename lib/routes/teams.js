...
Router.route("/teams/:_id/about", {
  ...
  waitOn() {
    return [Meteor.subscribe("teams"), Meteor.subscribe("users", Meteor.userId()), Meteor.subscribe("images")];
  },
...
Router.route("/teams/:_id", {
  ...
  waitOn() {
    return [Meteor.subscribe("teams"), Meteor.subscribe("users", Meteor.userId())];
  },

  action() {
    var userId = Meteor.userId();
    if (TeamUtils.isMember(userId, this.params._id) || Roles.userIsInRole(userId, [ADMIN, LECTURER, TA, JTA, SCRUM])) {
      this.render('team');
    } else {
